...
Por ejemplo: "Gimnasios/gimnasios_kanto.txt" (En este archivo hay 8 gimnasios) o "Gimnasios/gimnasio_1.txt" (En este archivo hay un solo gimnasio).
...
/*******************/
 ...
 -Para lo intercambios de pokemon, lo que se hace es solicitar al usuario que ingrese primero el ID del pokemon del equipo que desea cambiar, y luego el id del pokemon obtenido por el que desea sustituir al primero. Teniendo estos datos, lo que se verifica es que en la lista de combate NO se encuentre el pokemon de la lista de obtenidos. Esto se hace mediante una funcion que verifique que cierto pokemon no se encuentra en la lista de combate, ya que de ahcerlo de la manera simple (pedir que el ID del pokemon obtenido sea mayor que 6), es posible que en un proximo cambio, sea posible "duplicar" los pokemones. Por ejemplo:
Si en el primer turno cambio al pokemon de equipo de ID 1, por el pokemon obtenido de ID 7. En el 2do turno podria cambiar al pokemon del equipo de ID 2 con el pokemon obtenido de ID 7 nuevamente y asi tener (dos pokemones de ID 2 en la lista de equipo).
...
*En caso de recibir un archivo con varios gimnasios y que alguno de los gimnasios este con un formato incorrecto, el programa cargara todos lo que haya llegado a leer hasta el momento del error.
